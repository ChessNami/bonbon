...
const SK = ({ showFormer, onCloseFormer }) => {
        ...
        try {
            ...
            const withImages = await Promise.all(
                (data || []).map(async (o) => {
                    ...
                    if (o.image_url) {
                        const path = o.image_url.split("/skofficials/")[1] || `public/sk_official_${o.id}.jpg`;
            ...
            const sortedCurrent = current.sort((a, b) => {
                const isChairman = p => /sk chairman|chairman|chairperson/i.test(p);
                if (isChairman(a.position) && !isChairman(b.position)) return -1;
                if (!isChairman(a.position) && isChairman(b.position)) return 1;
    ...
    const chairman = officials.find(o => /sk chairman|chairman|chairperson/i.test(o.position));
    const others = officials.filter(o => o.id !== chairman?.id);
        ...
        <div className="p-4 sm:p-6 lg:p-8">
                ...
                <div className="space-y-12">
                    {chairman && (
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex justify-center">
                            <div
                                className="bg-white rounded-2xl shadow-xl overflow-hidden max-w-sm w-full cursor-pointer border-4 border-green-600"
                                onClick={() => setSelectedOfficial(chairman)}
                            >
                                <div className="relative aspect-square">
                                    <img src={chairman.signedImageUrl} alt={chairman.name} className="w-full h-full object-cover" />
                                    <div className="absolute top-4 right-4 bg-green-600 text-white font-bold px-4 py-2 rounded-full text-sm shadow">
                                        CURRENT
                                    </div>
                                </div>
                                <div className="p-6 text-center">
                                    <h3 className="text-2xl font-bold text-gray-800">{chairman.name}</h3>
                                    <p className="text-lg text-green-600 font-medium mt-2">{chairman.position}</p>
                                    <p className="text-sm text-gray-600 mt-3">
                                        Term: {chairman.start_year} – Present
                                    </p>
                                </div>
                            </div>
                        </motion.div>
                    ...
                    >
                        ...
                        >
                            <div className="sticky top-0 bg-white border-b px-6 py-4 flex justify-between items-center">
                                <h2 className="text-2xl font-bold">Former SK Chairman</h2>
                                <button onClick={onCloseFormer}><FaTimes className="w-6 h-6" /></button>
                            </div>
                            <div className="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                {formerOfficials.length === 0 ? (
                                    <p className="col-span-full text-center text-gray-500 py-10">No former leaders recorded.</p>
                                ) : (
                                    formerOfficials.map(o => (
                                        <motion.div
                                            ...
                                            onClick={() => { setSelectedOfficial(o); onCloseFormer(); }}
                                        >
            ...
            <AnimatePresence>
                {selectedOfficial && (
                    ...
                    >
                        <motion.div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm sm:max-w-md overflow-hidden"
                            initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
                            onClick={e => e.stopPropagation()}
                        >
                            <div className="relative h-64 sm:h-80">
                                <img src={selectedOfficial.signedImageUrl} alt={selectedOfficial.name} className="w-full h-full object-cover" />
                                <button onClick={() => setSelectedOfficial(null)} className="absolute top-3 right-3 p-2.5 bg-white rounded-full shadow">
                            ...
                            <div className="p-8 text-center">
                                ...
                                <p className="text-xl text-green-600 font-medium mt-2">{selectedOfficial.position}</p>
