        ...
        try {
            ...
            const membersWithSignedUrls = await Promise.all(
                (data || []).map(async (member) => {
                    if (member.image_url) {
                        const filePath = member.image_url.split('/skofficials/')[1] || `public/sk_official_${member.id}.jpg`;
    ...
    return (
        <div className="container mx-auto p-4 sm:p-6 md:p-8">
            {isLoading ? (
                ...
                <p className="text-center text-gray-500 sm:text-lg">No SK Officials data.</p>
            ) : (
                <div className="grid gap-4">
                    {/* First row: Only SK Chairman centered */}
                    <div className="flex justify-center">
                        {teamMembers
                            .filter(member =>
                                member.position === "SK Chairman" ||
                                member.position === "SK Chairperson"
                            )
                            .map((leader) => (
                                <div
                                    key={leader.id}
