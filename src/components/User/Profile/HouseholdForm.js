...
const HouseholdForm = ({ data, onNext, onBack, userId }) => {
    ...
    const fetchUserData = useCallback(async () => {
        ...
        try {
            ...
            if (data && data.household) {
                setFormData(data.household);
                setGender(data.household.gender || "");
                setCustomGender(data.household.customGender || "");
                setEmploymentType(data.household.employmentType || "");
            } else {
                setFormData({});
            }
        } catch (error) {
    ...
    const handleSubmit = async () => {
        const requiredFields = [
            "firstName", "lastName", "address", "region", "province", "city", "barangay",
